...
class Sq5MIDIService(BaseMidiService):
    ...
    def set_connection_params(self, ip: str, port: int, channel: int, use_tcp: bool = True) -> None:
        ...
        self.logger.info(f"SQ-5/6/7 연결 설정: {ip}:{port}, 채널:{channel}, TCP/IP:{use_tcp}")
    
    def connect(self) -> bool:
        """Connect to SQ-5 mixer."""
        with self._connection_lock:
            if self.sq5_connected:
                self.logger.info("SQ-5/6/7가 이미 연결되어 있습니다")
                return True
            ...
            try:
                ...
                self.logger.error(f"❌ SQ-5/6/7 연결 실패: {e}")
                return False
    
    def _connect_tcp_midi(self) -> bool:
        """Connect to SQ-5 via TCP/IP MIDI."""
        try:
            self.logger.info(f"🔍 SQ-5/6/7 TCP/IP MIDI 연결 시도: {self.sq5_ip}:{self.sq5_port}")
            ...
            self.logger.info(f"🎉 SQ-5/6/7 TCP/IP MIDI 연결 성공: {self.sq5_ip}:{self.sq5_port}")
            return True
    ...
    def _connect_usb_midi(self) -> bool:
        ...
        try:
            self.logger.info("🔍 SQ-5/6/7 USB MIDI 연결 시도...")
            ...
            self.sq5_connected = True
            self.logger.info("🎉 SQ-5/6/7 USB MIDI 연결 성공 (시뮬레이션)")
            return True
    ...
    def disconnect(self) -> None:
        ...
        with self._connection_lock:
            ...
            self.sq5_connected = False
            self.logger.info("SQ-5/6/7 믹서 연결 해제됨")
    ...
    def send_midi_message(self, message) -> bool:
        ...
        with self._connection_lock:
            if not self.sq5_connected:
                self.logger.warning("⚠️ SQ-5/6/7에 연결되지 않음")
                return False
            ...
            try:
                ...
                self.logger.error(f"❌ SQ-5/6/7 MIDI 전송 실패: {e}")
    ...
    def handle_mute(self, note: int, velocity: int, channel: int, mixer_midi_channel: int = None) -> None:
        ...
        self.logger.info(f"🔇 SQ-5/6/7 뮤트 제어: 채널 {channel_num}, 뮤트: {mute_on_off}, MIDI 채널: {midi_channel}")
    ...
    def handle_scene(self, note: int, channel: int, mixer_midi_channel: int = None) -> None:
        ...
        scene_number = note + 1
        self.logger.info(f"🎬 SQ-5/6/7 씬 리콜: {scene_number}번 씬, MIDI 채널: {midi_channel}")
    ...
    def handle_softkey(self, note: int, channel: int, mixer_midi_channel: int = None) -> None:
        ...
        self.logger.info(f"🔘 SQ-5/6/7 소프트키 제어: {softkey_number}번 소프트키 (0-based), MIDI 채널: {midi_channel}")
    ...
    def send_nrpn_mute_sequence(self, channel_num: int, mute_value: int, mixer_midi_channel: int = None) -> None:
        ...
        try:
            ...
            self.logger.info(f"🔇 SQ-5/6/7 {channel_num}번 채널 {action} 완료")
            
        except Exception as e:
            self.logger.error(f"❌ SQ-5/6/7 NRPN 뮤트 시퀀스 실패: {e}")
    ...
    def send_softkey_command(self, softkey_number: int, mixer_midi_channel: int = None) -> None:
        ...
        try:
            ...
            if ok_on and ok_off:
                self.logger.info(f"🔘 SQ-5/6/7 소프트키 트리거 완료: idx={softkey_number}, note=0x{midi_note:02X}")
            else:
                self.logger.error("❌ SQ-5/6/7 소프트키 Note On/Off 전송 실패")
            
        except Exception as e:
            self.logger.error(f"❌ SQ-5/6/7 소프트키 명령 실패: {e}")
    ...
    def recall_scene_by_number(self, scene_number: int, mixer_midi_channel: int = None) -> None:
        ...
        try:
            ...
            if self.send_midi_message(program_msg):
                self.logger.info(f"🎬 SQ-5/6/7 {scene_number}번 씬 리콜 완료 (PC={scene_number - 1})")
            else:
                self.logger.error("❌ Program Change 전송 실패")
            
        except Exception as e:
            self.logger.error(f"❌ SQ-5/6/7 씬 리콜 실패: {e}")
    ...
    def shutdown(self) -> None:
        ...
        self.disconnect()
        self.logger.info("SQ-5/6/7 MIDI 서비스 종료")
